...
if(~isempty(jsonStruct.data_streams))
	...
	for n = 1:nDataStreams
		data.Stream(n).TimestampOffset = convertQasperSerialDateNumber(jsonStruct.data_streams(n).timestamp_offset); %convert serial date number to matlab format (fractional days since 01/01/2000)
...
if(~isempty(jsonStruct.log_entries))
	for n=1:length(jsonStruct.log_entries)
		...
		data.LogEntries(n).LogTime = convertQasperSerialDateNumber(jsonStruct.log_entries(n).log_time); %convert log timestamp serial date number to matlab format (fractional days since 01/01/2000)
